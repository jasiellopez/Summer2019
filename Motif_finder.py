        ...
        for filename in files_dict:
            ...
            for name in files_dict[filename]:
                ...
                if len(window_in_list[motif][filename][name]) > 0:
                    #finds 10 most common motifs of a previously specified size in list of strings within window
                    co_motifs = FindRandMotifs(window_in_list[motif][filename][name], motif_size)
                    print("List ")
            ...
            if upstream >= (2*window) and not1st_downstream >= (2*window):
                ...
                if not1st_downstream >= window:
                    ...
                    if not1st_downstream > 0:
                        ...
                        curr_window = sequence1[start - window - curr_motif_size:start - curr_motif_size]
                        curr_window1 = ""
                window_in_list[motif][filename][name].append("2a" + str(index) + str(not1st_downstream) + curr_window + str(sequence1[start - curr_motif_size: start]))
                window_in_list[motif][filename][name].append("2b" + curr_window1)
...
def parseSeqs(filename, sequences_dict):
    ...
    while i < len(sequence_as_list):
        if len(sequence_as_list[i]) > 0 and i < (len(sequence_as_list) - 1) and sequence_as_list[i][0] != '>':
